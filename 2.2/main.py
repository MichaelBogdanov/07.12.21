...
# Инициализируем файлы с выходными данными
file = open("discovered.txt", "a+");
output_movie = cv2.VideoWriter(f"{now}.outputVideo.mp4", cv2.VideoWriter_fourcc(*'mp4v'), video_capture.get(cv2.CAP_PROP_FPS), (int(video_capture.get(cv2.CAP_PROP_FRAME_WIDTH)), int(video_capture.get(cv2.CAP_PROP_FRAME_HEIGHT))))
